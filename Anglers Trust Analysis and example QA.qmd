...
If there are conditions which tend to skew results we can apply a confidence score to this data and populate a new column. High river levels are interpreted to result in turbid conditions which can skew off-the-shelf colorimeter results which are designed for use in aquariums, but offer a cheaper alternative for Citizen Science water quality projects. Temperatures less than 10°C also tend to skew these off-the-shelf meters, therefore we can apply a confidence column to this data.
...
DT::datatable(head(joined[,c(2:15, 32,37,38,40)]))
