...
This document outlines how we can automate the QAQC and transformation of citizen science data.

By utilising our partner's data, the EA can enhance its understanding of our catchments. However, third-party schemes should undergo a layer of QA/QC upon arrival into the EA. This means our EA analysts and decision-makers can be more confident in the accuracy of the data we receive from our partners, enabling more informed decisions where margins of error are constrained.

## Introduction

Here we use the [Angling Trust Water Quality Monitoring Network](https://anglingtrust.net/wp-content/uploads/2022/12/Water-Quality-Monitoring-Network-Viewers-Pack-v2-FINAL-09.12.2022.pdf) (ATWQN) as an example citizen science scheme which we can QA and transform to then make available to the wider EA. The ATWQN uses Epicollect5 as a data collection platform. This platform enables data to be shared via manual download, visualises sample points on a map in app and importantly, like some other Citizen Science platforms provides an open-access API: enabling data to consistently be shared in a uniform format and in near-real-time.
...
If there are conditions which tend to skew results we can apply a confidence score to this data and populate a new column. High river levels are interpreted to result in turbid conditions which can skew off-the-shelf colorimeter results which offer a cheaper alternative for Citizen Science water quality projects. Temperatures less than 10°C also tend to skew these off-the-shelf meters, therefore we can apply a confidence column to this data when these results are recorded.
...
DT::datatable(head(today[,c(4:8,30)]))
...
DT::datatable(head(joined[,c(2:15, 32,36:40)]))

```

We can also link citizen science data: it may be useful to link additional lines of evidence adding resolution to cit sci data. Here we link ATWQMN data with Riverfly Monitoring Initiative (RMI) data, although these are from different organisations, this can be carried our in-real-time and surveys linked by a unique identifier based on their proximity. Here surveys within a 50m radius are paired.

The below map shows sites which have associated RMI and ATWQMN data. Zoom in to explore this data. Data hasn't been filtered for surveys to be on the same date, although we could do this.
...
leaflet() %>% 
  ...
  addCircleMarkers(data= df_sf,
                   col = "blue",
                   radius = 4,
                    popup = paste0("Date of Survey: ", df_sf$Date, 
                                  "<br>Time: ", df_sf$`_At_what_time_was_t`,
                                  "<br>River: ", df_sf$River_Name,
                                  "<br>Phosphate: ", df_sf$Phos_ppm,
                                  "<br>EC: ", df_sf$`_What_is_the_Elect`),
                   group = "All ATWQMN Data") %>% 
   addCircleMarkers(data=ARMI,
                   col="seagreen2",
                   radius=3,
                   popup = paste0("Date of Survey: ", ARMI$Recorded..Date,
                                  "<br>Time: ", ARMI$Recorded..Time,
                                  "<br>River: ", ARMI$River,
                                  "<br>Score: ", ARMI$ARMI.Total),
                   group = "All ARMI Data") %>% 
  addLayersControl(overlayGroups = c( "All ATWQMN Data",
              "All ARMI Data",
              "ATWQMN Paired Data",
              "ARMI Paired Data"),
                   position = "topright",
                   options= layersControlOptions(collapsed=FALSE)) %>% 
  hideGroup(c("All ATWQMN Data",
              "All ARMI Data")) %>% 
  addLegend(
...
Once the data has arrived at the EA and it has automatically undergone QAQC, we can store this transformed data on a national database which EA analysts are provided with access to. From here they can inform our catchment planning without the time-consuming transformation of data by individual analysts each time they get new citizen science data. Or with different EA area approaches to QAQC.
