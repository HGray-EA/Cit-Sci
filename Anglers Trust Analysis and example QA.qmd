...
If there are duplicate surveys, for example, if a surveyor accidentally submits results twice. These can be screened and removed.
...
Additional calculations can be automatically applied to data when it arrives at the EA. This could be the application of a conversion factor, for example, to convert from one chemical species to another; converting easting and northings to lat and long; or applying means etc. The below map explores some of the ATWQM nitrate data. The below mean covers an arbitrary time span, a mean is calculated from `r range(df$Date)[1]` to today's date.
